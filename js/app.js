...
(() => {
  ...
  const KEY = "u5_correspondencia_progress_v1";
  ...
  if (resetBtn) {
    resetBtn.addEventListener("click", () => {
      ...
      Object.keys(localStorage)
        .filter(k => k.includes("u5_correspondencia_progress") || k.startsWith("teacher_unlock_") || k.toLowerCase().includes("teachergate"))
  ...
  function renderQuiz(phaseKey, mountId) {
    ...
    mount.innerHTML = bank.map((item, qi) => {
      ...
      return `<div class="quiz-item"><p><strong>${qi + 1}.</strong> ${item.q}</p>${opts}</div>`;
    }).join("");
  }
  ...
  function handleSubmit(phaseKey, resultId) {
    ...
    if (pct >= 80) {
      ...
      save(); refreshUI(); showView(nextViewOf(phaseKey));
    } else {
  ...
  function applyGateState(phaseNum) {
    ...
    if (lockMsg) {
      lockMsg.textContent = unlockedToday ? "" : "Test bloqueado. Introduce el código del docente para habilitar la corrección.";
    }
  }
